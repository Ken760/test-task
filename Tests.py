...
def test_search_in_yandex(browser):
    ...
    with allure.step("Проверка, что появилась таблица с подсказками (suggest)"):
...
def test_yandex_images(browser):
    ...
    with allure.step("Проверка, что перешли на url https://yandex.ru/images/"):
    ...
    with allure.step("При нажатии кнопки вперед  картинка изменяется,"
                     "При нажатии кнопки назад картинка изменяется на тоже изображение"):
